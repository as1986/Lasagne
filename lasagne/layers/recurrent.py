...
class GRUNLLayer(MergeLayer):
    ...
    def __init__(self, incoming, num_units,
                 ...
                 hidden_update=Gate(W_cell=None,
                                    nonlinearity=nonlinearities.tanh),
                 hid_init=init.Constant(0.),
                 nl_update_w=init.Normal(.1),
                 nl_update_b=init.Constant(0.),
                 nl_update_type=nonlinearities.tanh,
                 backwards=False,
                 learn_init=False,
                 gradient_steps=-1,
                 grad_clipping=0,
        ...
        # Initialize parent layer
        super(GRUNLLayer, self).__init__(incomings, **kwargs)
        ...
        self.nl_update_w = self.add_param(nl_update_w, (self.num_units, self.num_units),
                                          name='nl_update_w')

        self.nl_update_b = self.add_param(nl_update_b, (self.num_units,),
                                          name='nl_update_b', regularizable=False)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(input_n, hid_previous, *args):
            ...
            # Compute (1 - u_t)h_{t - 1} + u_t c_t
            pre_nl = updategate * hidden_update
            hid = (1 - updategate)*hid_previous + self.nl_update_type(self.nl_update_w * pre_nl + self.nl_update_b)
            return hid
...
class GRULayerAtt(MergeLayer):
    ...
    operations in a single dot product.
    """
    def __init__(self, incoming, encoded, encoded_mask, num_units,
                 resetgate=Gate(W_cell=None),
                 updategate=Gate(W_cell=None),
                 hidden_update=Gate(W_cell=None,
                                    nonlinearity=nonlinearities.tanh),
        ...
        # FIXME
        assert precompute_input is False

        incomings = [incoming, encoded, encoded_mask]
        self.encoded_incoming_index = 1
        self.encoded_mask_index = 2
        self.mask_incoming_index = -1
        self.hid_init_incoming_index = -1
        if mask_input is not None:
            incomings.append(mask_input)
        ...
        # Initialize parent layer
        super(GRULayerAtt, self).__init__(incomings, **kwargs)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        # Create single recurrent computation step function
        # input__n is the n'th vector of the input
        def step(input_n, hid_previous, att_previous, *args):
            ...
            input_n = T.concatenate([input_n, att_previous], axis=1)
            if not self.precompute_input:
                # Compute W_{xr}x_t + b_r, W_{xu}x_t + b_u, and W_{xc}x_t + b_c
                input_n = T.dot(input_n, W_in_stacked) + b_stacked
            ...
            masked_attention_exp = T.switch(T.neq(encoded_mask, 0),
                                            T.exp(ut - T.max(ut, axis=1, keepdims=True)).astype('float32'),
                                            T.constant(0., dtype='float32'))
            masked_attention_normalized = masked_attention_exp / T.sum(masked_attention_exp, axis=1,
                                                                       keepdims=True).astype('float32')
            att = T.sum(masked_attention_normalized.dimshuffle(0,1,'x') * encoded, axis=1)
            return hid, att

        def step_masked(input_n, mask_n, hid_previous, att_previous, *args):
            hid, att = step(input_n, hid_previous, att_previous, *args)
...
class DynamicGRULayer(MergeLayer):
...
class LSTMLayer_withCellOut(MergeLayer):
    r"""
    lasagne.layers.recurrent.LSTMLayer(incoming, num_units,
    ingate=lasagne.layers.Gate(), forgetgate=lasagne.layers.Gate(),
    cell=lasagne.layers.Gate(
    ...
    def __init__(self, incoming, num_units,
        ...
        # Initialize parent layer
        super(LSTMLayer_withCellOut, self).__init__(incomings, **kwargs)
